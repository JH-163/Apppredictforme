...
with col1:
    ...
    Surgical_modality = st.selectbox("Surgical modality", ("celioscope", "laparotomy"))
with col3:
    Surgical_duration = st.number_input("Surgical duration (hour)", min_value=0.0, value=5.0)
    Intraoperative_blood_loss = st.number_input("Intraoperative blood loss (ml)", min_value=0.0, value=200.0)
    Hospital_stay_duration = st.number_input("Hospital stay duration (day)", min_value=0.0, value=5.0)

with col2:
    Postoperative_analgesia = st.selectbox("Postoperative analgesia (Oral analgesics/Analgesic pumps)", ("Yes", "No"))
    Types_of_anesthesia = st.selectbox("Types of anesthesia", ("neuraxial anesthesia", "general anesthesia"))
    POP_Q = st.selectbox("Pelvic organ prolapse quantification", ("0", "1", "2", "3"))
...
if st.button("Predict"):
    ...
    min_max_values = {
        ...
        "Intraoperative_blood_loss": (1, 3000),
    ...
    Surgical_duration = minmax_scale(Surgical_duration, *min_max_values["Surgical_duration"])
    Intraoperative_blood_loss = minmax_scale(Intraoperative_blood_loss, *min_max_values["Intraoperative_blood_loss"])
    ...
    bool_columns = ["Age", "Menopause", "Surgical_modality", "Postoperative_analgesia", 
                    "Types_of_anesthesia"]
    
    X = pd.DataFrame([[Age, Menopause, Surgical_modality, Postoperative_analgesia, Types_of_anesthesia, POP_Q, 
                       Surgical_duration, Intraoperative_blood_loss,Hospital_stay_duration]],
    ...
    X["POP_Q"] = X["POP_Q"].replace({"0": 0, "1": 1, "2": 2, "3": 3})
